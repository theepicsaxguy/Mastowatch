...
class TestCeleryTasks(unittest.TestCase):
    ...
    def test_analyze_and_maybe_report_dry_run(self, mock_bot_client, mock_settings, mock_rule_service, mock_db):
        ...
        mock_rule_service.evaluate_account.return_value = [
            Violation(
                rule_name="rule1", score=0.5, evidence=Evidence(matched_terms=[], matched_status_ids=[], metrics={})
            ),
            Violation(
                rule_name="rule2", score=0.3, evidence=Evidence(matched_terms=[], matched_status_ids=[], metrics={})
            ),
        ]
