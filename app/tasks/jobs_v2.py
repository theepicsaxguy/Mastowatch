...
from app.metrics import (
    analyses_flagged, reports_submitted, accounts_scanned, 
    analysis_latency, report_latency, cursor_lag_pages, queue_backlog
)
...
@shared_task(name="app.tasks.jobs_v2.poll_admin_accounts",
...
def poll_admin_accounts():
    ...
    while pages < settings.MAX_PAGES_PER_POLL:
        ...
        response = admin.get_admin_accounts(
            origin="remote",
            status="active", 
...
@shared_task(name="app.tasks.jobs_v2.analyze_and_maybe_report_v2",
             autoretry_for=(Exception,), retry_backoff=2, retry_backoff_max=60, retry_jitter=True)
def analyze_and_maybe_report_v2(payload: dict):
    ...
    try:
        ...
        try:
            ...
            statuses_typed = admin.get_account_statuses(
                account_id=acct_id,
            ...
            sr = admin.get(f"/api/v1/accounts/{acct_id}/statuses", 
                          params={"limit": settings.MAX_STATUSES_TO_FETCH})
            statuses = sr.json()
        ...
        dedupe = make_dedupe_key(
            acct_id, status_ids, settings.POLICY_VERSION, 
            rules.ruleset_sha256, {"hit_count": len(hits)}
        )
...
def analyze_specific_account(account_id: str):
    ...
    try:
        ...
        for status in statuses:
            logging.info(f"Status {status.id}: {len(status.content)} chars, "
